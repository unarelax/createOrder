...
var Receipt = (function () {
  ...
  // 数量和金额都按 2 位收敛后去掉多余的 0，2.50 显示成 2.5
  function fmtNum(n) {
  ...
  function detailText(line) {
    return fmtNum(line.qty) + (line.unit || '') + ' × ' + fmtNum(line.price) + '元';
  }
  ...
  function buildMeta(order) {
    var meta = [];
    if (order.no) meta.push({ label: '单号', value: order.no });
    ...
    return {
      ...
      items: lines.map(function (l) {
        return {
          ...
          price: pricePerUnit(l.price, l.unit),
  ...
  function toText(order) {
    ...
    lines.forEach(function (l, i) {
      ...
      out.push(
        (i + 1) + '.' + l.name + '：' +
        fmtNum(l.qty) + unit + '，' +
        pricePart + '，共' + fmtNum(amountOf(l)) + '元'
      );
    });
    ...
    if (order.no) out.push('单号：' + order.no);
